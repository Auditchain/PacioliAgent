...
async function ValidationInitialized2filters(filter1, filter2) {
    ...
    try {
        ...
        console.log(err); impl_contract = Web3.toHex(
            web3.eth.get_storage_at(
                contract_address,
...
app.get('/ValidationInitializedEthers', function (req, res) {

    let filter = req.query.filter;

    ValidationInitializedEthers(filter).then(async function (returnedData) {
        res.end(JSON.stringify(returnedData));
    }).catch(function (err) {
        console.log(err);
    })
